   ...
   if(!isEmpty(enquiryTypeCode)){
	if(enquiryTypeCode=='CP'){
		...
		if(!isEmpty(channelPartner) && !isEmpty(brokerContact) && enq.walkInSource==='Channel Partner'){
			...
			|| enq.walkInSource==='SMS' || enq.walkInSource==='Referral' || enq.walkInSource==='Godrej Employee' || enq.walkInSource==='Corporate' || enq.walkInSource==='Existing Customer' || enq.walkInSource==='Other BTL activities' || enq.walkInSource==='Affiliate Sales' || enq.walkInSource==='Telemarketing' || enq.walkInSource==='VDNB Referral'	
