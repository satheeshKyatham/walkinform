...
$(document).ready(function(){
   ...
   // $('#mainPageLoad').hide();
	$("#enquiryRequestCPBasicInfoForm").find("input,select,textarea").attr("tabIndex","-1");
	 $(this).scrollTop(0);
});
function onPageLoad(){
	//debugger	
...
function getExistingInfoByMobileAndProject(){
	...
	if(project!=""){	
		...
		var resp=fetchData("getExistingCPRecords/"+countryCode+"/"+mobileNo+"/"+project,null,"GET");
		//debugger
...
function saveCPBaseInfo(event,el){
...
*/	submitIt("enquiryRequestCPBasicInfoForm","saveCPBaseInfo","savebasicInfoResp");
...
//        	window.location.href=url+"/"+"generateWalkinToken"+"?enquiryid="+enq.enquiryId+"&mobileno="+code+"&projectSFID="+$("#projectSfid").val()+"&projectName="+$("#projectName").val();
        	//window.location.href=url+"/"+"generateWalkinToken"+"?enquiryid="+enq.enquiryId+"&mobileno="+code+"&projectSFID="+$("#projectSfid").val()+"&projectName="+$("#projectName").val();
            debugger;
        	/*$.get(url+"/generateWalkinToken",{"enquiryid":enq.enquiryId,"mobileno":contact.mobileNo,"projectSFID":$("#projectSfid").val(),"projectName":$("#projectName").val()},function(data){				 
        	}).done(function(data){
        	});*/
			...
			swal({
		        ...
		        if (isConfirm) {
		        	window.location.href=url+"/cpsuccess";
		        	console.log('OK');
