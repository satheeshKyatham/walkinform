...
function getTokenTypeEOI (){
	...
	} else if($("#projectsfid").val()=='a1l2s000000XoezAAC' || $("#projectsfid").val()=='a1l6F000004RvPHQA0' || $("#projectsfid").val()=='a1l2s000000PKrrAAG') {
		html='<option value="F">PLATINUM</option><option value="T">EXPRESS</option>';
...
function getTypologyEOI (e){
	...
	if ($('.projectSfid').val() == "a1l2s00000002YZAAY") {
		...
		if ($(e).closest('.EOIDtlRow').find('.towerListEOI  option:selected').val() == "SC04") {
			html = "<option value='3 BHK'>3 BHK</option> <option value='4 BHK'>4 BHK</option>";
		} else if ($(e).closest('.EOIDtlRow').find('.towerListEOI  option:selected').val() == "SC05") {
