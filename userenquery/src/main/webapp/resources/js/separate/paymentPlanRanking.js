	...
	$.getJSON(
			...
			function(data) {
				...
				$.each(data, function(index, value) {
					$('#regionList').append(
							"<option value='" + value.region__c + "'>"
									+ value.region__c + "</option>");
				});
			}).done(function() {
	});
}
...
function projectDataList() {
	...
	var urlTower = pageContext + "projectDataList?region="
			+ $('#regionList').val();
	$.getJSON(
			urlTower,
			function(data) {
				...
				$.each(data, function(index, value) {
					$('#projectDataList').append(
							"<option value='" + value.sfid + "'>" + value.name
									+ " / "
									+ value.propstrength__project_code__c
									+ "</option>");
				});
			}).done(function() {
...
function paymentPlanDropdown(data) {
	...
	for (i = 0; i < data.length; i++) {
		...
		var planName = '';
		var sfid = '';
		sfid = data[i].payment_plan_sfid == undefined ? ''
				: data[i].payment_plan_sfid;
		planName = data[i].payment_plan_name == undefined ? ''
				: data[i].payment_plan_name;
		var existrankNumber = data[i].sequence == undefined ? ''
				: data[i].sequence;
		for (j = 0; j < totalPP; j++) {
			var d = j + 1;
			rankNumber += '<option value=' + d + '  class="rankClass">' + d
					+ '</option>';
		}
		if (planName == "") {
		...
		html += '<tr class="paymentRankDataPlotRow data-rowid = "'
				+ data[i].sfid
				+ '">'
				+ '<td style="text-align:center;">'
				+ '<input style="display:none;" type="hidden" class="oldPaymentPlan" id="oldPaymentPlanId"  value="'
				+ sfid
				+ '"></input>'
				+ '<input style="display:none;" type="hidden" class="oldPaymentPlan2" id="oldPaymentPlanId2"  value="'
				+ planName
				+ '"></input>'
				+ '<span class="existPaymentPlan">'
				+ planName
				+ '</span>'
				+ '</td>'
				+ '<td style="text-align:center;">'
				+ '<select style="display:none;" class="form-control showRank rankNo'
				+ i + '" id="rank"> ' + rankNumber + '</select>'
...
function bulkSubmitPaymentRanking() {
	...
	if (checkDupli) {
		...
		$("#mainPageLoad").show();
		var arrayData = [];
		$("#paymentRankListId .paymentRankDataPlotRow").each(
				function() {
					var rankData = {};
					rankData.payment_plan_sfid = $(this)
							.find('.oldPaymentPlan').val();
					rankData.payment_plan_name = $(this).find(
							'.oldPaymentPlan2').val();
					rankData.sequence = $(this).find('#rank').val();
					rankData.isactive = 'A';
					rankData.createdby = '999';
					rankData.project_name = $(
							'#projectDataList option:selected').text();
					rankData.project_sfid = $('#projectDataList').val();
					arrayData.push(rankData);
				});
		debugger
		$.post(pageContext + "bulkInsertPaymentRanking", {
			"rankingJson" : JSON.stringify(arrayData)
		}, function(data) {
		}).done(function(data) {
			debugger
...
function getPaymentPlanData() {
	$("#mainPageLoad").show();
	$.get(
			pageContext + "getProjectPPRanking?projectcode="
					+ $("#projectDataList").val(), function(data) {
			}).done(function(obj) {
		if (obj.length == 0) {
...
function getPaymentPlanWithCip() {
	$.get(
			pageContext + "getpaymentPlanWithCIP?projectcode="
					+ $("#projectDataList").val(), function(data) {
			}).done(function(obj) {
		paymentPlanDropdown(obj);
