...
function addTnCCharge () {
	if($('#projectDataList').val() != '') {
		 
		$.post(pageContext+"insertTnCForPP",{"tnc_text":$('.Editor-editor').html(),  "project_id":$('#projectDataList').val(), "project_name":$('#projectDataList :selected').text(), "pymt_plan_id":$('#ppDropdown').val(), "pymt_plan_name":$('#ppDropdown :selected').text(), "region_id":$('#region_id').val(), "region_name":$('#region_id').val(),"tower_sfid":$('#towerMst').find('option:selected').attr('name')   },function(data){				 
