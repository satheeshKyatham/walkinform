...
public class InventoryDaoImpl extends AbstractDao<Integer, Inventory> implements InventoryDao{
	...
	public List<Inventory> getUnitDtl(String project_code, String towerMst, String typoMst, String holdMst, String soldMst,String facing, String unitAvailable, String unitCategory) {	 
		...
		List<Inventory> list =session.createQuery(" FROM  Inventory  where propstrength__project_name__c='"+project_code+"' and  tower_code__c='"+towerMst+"' "+ typology +" "+selectedFacing+"  "+category+" "+unitAvailbleCondition+"  and propstrength__active__c='t' ORDER BY floor_number__c, propstrength__house_unit_no__c, wing_block__c").list();
