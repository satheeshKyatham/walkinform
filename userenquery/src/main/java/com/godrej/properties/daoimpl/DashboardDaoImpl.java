...
public class DashboardDaoImpl implements DashboardDao{
	...
	public DashboardReport getDashboard(String projectSFID, String userid) {
		...
		Query totalPendingLeadISS = session.createNativeQuery("select count(nv_token_id) as totalPendingLead FROM salesforce.nv_token where isactive = 'Y' AND window_assign = '"+userid+"' and projectname= '"+projectSFID+"' AND (isdone = 'S' OR isdone is null)  ");
		...
		alotMIS.setTotalPendingLead(totalPendingLeadISS.getResultList().get(0) == null ? "0" : totalPendingLeadISS.getResultList().get(0).toString().toString());
