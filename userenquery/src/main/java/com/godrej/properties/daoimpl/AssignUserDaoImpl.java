...
public class AssignUserDaoImpl extends AbstractDao<Integer, AssignedUser> implements AssignUserDao {
	...
	public List<AssignedUser> getassignedusers(String user_id,String projectId,String fromdate,String todate, String source) {
		...
		if (source.equals("TOTALPENDING")) {
			...
			whereCondition = "a.isactive = 'Y' AND window_assign = '"+user_id+"' and projectname= '"+projectId+"' AND (a.isdone = 'S' OR a.isdone is null) order by isdone asc";
		} else {
	...
	public List<TodaysFollowUp> getData(String user_id,String projectId,String fromdate,String todate, String source) {
		...
		whereCondition = "a.isactive = 'Y' AND window_assign = '"+user_id+"' and projectname= '"+projectId+"' and Date(nvreq.followdate) between '"+fromdate+"' and '"+todate+"' order by isdone asc";
		...
		Query q = session.createNativeQuery(" SELECT row_number() OVER (ORDER BY a.nv_token_id) AS row_number,  "
			...
			+ " nvreq.followtype, "
			+ " nvreq.followdate, "
			+ " CASE WHEN req.name IS NULL THEN '' "
				+ " ELSE req.name "
			+ " END AS enqname,  "
			+ " req.rating__c  "
			...
			+ " LEFT JOIN salesforce.gpl_cs_balance_details bdetails ON req.sfid = bdetails.enquiry_sfid AND bdetails.isactive = 'A' "
			+ " LEFT JOIN salesforce.propstrength__offer__c offername ON bdetails.offer_sfid = offername.sfid AND offername.propstrength__status__c = 'Closed Won' "
			//+ " WHERE a.isactive = 'Y' AND window_assign = '"+user_id+"' and projectname= '"+projectId+"' and Date(a.created) between '"+fromdate+"' and '"+todate+"' order by isdone asc ", TodaysFollowUp.class);
			+ " WHERE "+whereCondition+" ", TodaysFollowUp.class);
